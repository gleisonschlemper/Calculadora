...
class calculadora{
    ...
    setValue = (value) =>  {
        body().innerHTML = '';
        this.digitos.push(value);
        this.getTela();
    }
    getTela = () => {
        body().innerHTML = '';
        this.digitos.map((digito)=>{
            body().innerHTML+= digito;
        })
    }
    getCalcular = () =>  {
        body().innerHTML = eval(this.digitos.join(''));
    }
    getDeletarUm = () => {
        this.digitos.pop();
        this.getTela();
    ...
    getDeletarTodos = () => { 
        body().innerHTML = ''; 
        for (let i = 0; i <= this.digitos.length; i++) {this.digitos.splice(0,i);}
        this.getTela();
    }  
}

var calc = new calculadora();
body = () => {return document.querySelector('.resultado');}
teclado = (value) => {calc.setValue(value);}
apagarUm = () => { calc.getDeletarUm();}
soma = () => {calc.getCalcular();}
apagarTudo = () => {calc.getDeletarTodos();}
